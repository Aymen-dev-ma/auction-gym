...
class RLAgent:
    def __init__(self, rng, name, num_items, item_values, allocator, bidder, memory=0, policy_net=None):
        self.rng = rng
        self.name = name
        ...
        self.bidder = bidder
        self.memory = memory
        self.policy_net = policy_net if policy_net else PolicyNetwork(input_size=4, output_size=num_items)

    def select_item(self, context):
        deconfounded_context = self.allocator.estimate_CTR(context)  # Update this line accordingly
        action, _ = self.policy_net.act(deconfounded_context)
        best_item = action
        estimated_CTR = deconfounded_context[best_item]
        return best_item, estimated_CTR
    ...
    def bid(self, context):
        ...
        self.logs.append(ImpressionOpportunity(context=context, item=best_item, estimated_CTR=estimated_CTR, value=value, bid=bid, best_expected_value=0.0, true_CTR=0.0, price=0.0, second_price=0.0, outcome=0, won=False))
