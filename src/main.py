...
def instantiate_agents(rng, agent_configs, agents2item_values, agents2items):
    ...
    for agent_config in agent_configs:
        vae = None
        if 'VAE' in agent_config['bidder']['type']:
            vae_params = agent_config['bidder']['kwargs']
            vae = VAELSTM(vae_params['vae_input_dim'], vae_params['vae_hidden_dim'], vae_params['vae_latent_dim'])
        agent = Agent(rng=rng,
