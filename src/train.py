...
def parse_config(path):
    ...
    agents2item_values = {agent_config['name']: rng.lognormal(0.1, 0.2, agent_config['num_items']) for agent_config in agent_configs}
...
def instantiate_agents(rng, agent_configs, agents2item_values, agents2items):
    agents = [Agent(rng=rng, name=agent_config['name'], num_items=agent_config['num_items'], item_values=agents2item_values[agent_config['name']], allocator=DeconfounderAllocator(rng=rng, embedding_size=5, num_items=agent_config['num_items']), bidder=RLAgent(rng=rng, name=agent_config['name'], num_items=agent_config['num_items'], item_values=agents2item_values[agent_config['name']], allocator=DeconfounderAllocator(rng=rng, embedding_size=5, num_items=agent_config['num_items']), bidder=None), memory=(0 if 'memory' not in agent_config.keys() else agent_config['memory'])) for agent_config in agent_configs]
    for agent in agents:
...
def instantiate_auction(rng, config, agents2items, agents2item_values, agents, max_slots, embedding_size, embedding_var, obs_embedding_size):
    return (Auction(rng, config['allocation'](), agents, agents2items, agents2item_values, max_slots, embedding_size, embedding_var, obs_embedding_size, config['num_participants_per_round']), config['num_iter'], config['rounds_per_iter'], config['output_dir'])
...
def simulation_run():
    for i in range(num_iter):
        ...
        for agent_id, agent in enumerate(auction.agents):
            ...
            agent2gamma[agent.name].append(torch.mean(torch.Tensor(agent.bidder.gammas)).detach().item() if not agent.bidder.truthful else np.mean(agent.bidder.gammas))
...
if __name__ == '__main__':
    ...
    plot_measure_per_agent(run2agent2CTR_bias, 'CTR Bias', optimal=1.0)
