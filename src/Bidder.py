...
class EmpiricalShadedBidder(Bidder):
    ...
    def update(self, contexts, values, bids, prices, outcomes, estimated_CTRs, won_mask, iteration, plot, figsize, fontsize, name):
        ...
        best_gamma = np.clip(best_gamma, 0, 1)
...
class VAELSTMBidder(Bidder):
    def __init__(self, rng, vae_input_dim, vae_hidden_dim, vae_latent_dim):
        super(VAELSTMBidder, self).__init__(rng)
        self.vae = VAELSTM(vae_input_dim, vae_hidden_dim, vae_latent_dim)

    def bid(self, value, context, estimated_CTR):
        context_tensor = torch.tensor(context, dtype=torch.float32).unsqueeze(0)
        confounders = self.vae.encode(context_tensor)[0].detach().numpy()
        bid = value * estimated_CTR * np.mean(confounders)
        return bid
