...
class EmpiricalShadedBidder(Bidder):
    ...
    def update(self, contexts, values, bids, prices, outcomes, estimated_CTRs, won_mask, iteration, plot, figsize, fontsize, name):
        ...
        best_gamma = max(0, min(1.0, best_gamma))
...
class VanillaRL(Bidder):
    def __init__(self, rng, model, optimizer, criterion, gamma=0.99):
    ...
    def bid(self, value, context, estimated_CTR):
        ...
        bid = value * estimated_CTR * action.item()
        return bid
...
class VAELSTMBidder(Bidder):
    def __init__(self, rng, vae, lstm):
        super(VAELSTMBidder, self).__init__(rng)
        self.vae = vae
        self.lstm = lstm

    def bid(self, value, context, estimated_CTR):
        state = torch.FloatTensor(context).unsqueeze(0)
        z, _, _ = self.vae(state)
        action_probs = self.lstm(z)
        action = torch.distributions.Categorical(action_probs).sample()
        bid = value * estimated_CTR * action.item()
        return bid
